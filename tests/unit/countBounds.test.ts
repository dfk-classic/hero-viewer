...
describe("count bounds", () => {
	it("pins MIN_COUNT and MAX_COUNT so the input attrs and clamp stay in sync", () => {
		// Guards against an accidental swap or drift between these constants and the values the input previously hard-coded (min={1} max={60}).
...
describe("clampCount", () => {
	...
	it("falls back to MIN_COUNT for empty, zero, negative, or non-numeric input", () => {
		// Mirrors the original `Number(value) || 1` then Math.max(1, …): "" and "abc" coerce to 0/NaN (falsy → MIN_COUNT), 0 is falsy, and negatives clamp up.
