...
// The flip toggle and the hero-id copy control were click-only divs, unreachable by keyboard users. These anchors guard the Enter/Space activation predicate that now backs their onKeyDown handlers, mirroring native <button> activation.
describe("isActivateKey", () => {
	...
	it("ignores keys that do not activate a button", () => {
		...
		// Guard against loose matching: trailing space and the legacy IE key name must not count as activation.
